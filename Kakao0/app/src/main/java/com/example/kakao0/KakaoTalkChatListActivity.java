...
//        msgType = findViewById(R.id.message_type);
    ...
    private ChatFilterBuilder makeChatFilterBuilder() {
        ...
        switch (position) {
            ...
            case 1:
                // 일반 1:1 채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.DIRECT).addFilter(ChatFilterBuilder.ChatFilter.REGULAR);
                break;
            case 2:
                // 일반 그룹채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.MULTI).addFilter(ChatFilterBuilder.ChatFilter.REGULAR);
                break;
            case 3:
                // 일반 채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.REGULAR);
                break;
            case 4:
                // 오픈 1:1 채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.DIRECT).addFilter(ChatFilterBuilder.ChatFilter.OPEN);
                break;
            case 5:
                // 오픈 그룹채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.MULTI).addFilter(ChatFilterBuilder.ChatFilter.OPEN);
                break;
            case 6:
                // 오픈 채팅방
                builder.addFilter(ChatFilterBuilder.ChatFilter.OPEN);
                break;
        }
        return builder;
    }
...
//        TalkMessageHelper.showSendMessageDialog(this, (dialog, which) -> requestSendMessage(MSG_TYPE.valueOf(msgType.getSelectedItemPosition()), chatInfo));
    ...
    private void requestSendMessage(FriendsMainActivity.MSG_TYPE msgType, ChatInfo chatInfo) {
